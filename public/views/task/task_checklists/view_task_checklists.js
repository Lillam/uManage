$(() => {
    ...
    $body.on("keyup", ".task_checklist_name", function (event) {
        ...
        if ($this.next().hasClass("uk-hidden")) $this.next().removeClass("uk-hidden");

        if ($this.html().trim() !== "" && $this.html().trim() !== cache().get(`task_checklist_${task_checklist_id}`)) {
            $this.next().find(".save_task_checklist_name").removeClass("disabled");
        } else {
            $this.next().find(".save_task_checklist_name").addClass("disabled");
        }

        if (event.ctrlKey && event.key === "Enter") $this.next().find(".save_task_checklist_name").trigger("click");
    });
    ...
    $body.on("click", ".cancel_task_checklist_name", function (event) {
        ...
        $this
            .parent()
            .prev()
            .html(cache().get(`task_checklist_${task_checklist_id}`));
        cache().remove(`task_checklist_${task_checklist_id}`);
    ...
    $body.on("blur", ".new_task_checklist_input", function (event) {
        let $this = $(this);
        if ($this.val() === "") $this.parent().next().addClass("uk-hidden");
    });
    ...
    $body.on("keyup", ".new_task_checklist_input", function (event) {
        ...
        if ($this.val() === "") $this.parent().next().find("a").addClass("disabled");

        if ($this.val() !== "") $this.parent().next().find("a").removeClass("disabled");

        if (event.key === "Enter") $this.parent().next().find("a").click();
    });
    ...
    $body.on("click", ".task_checklist_zip", function (event) {
        let $this = $(this),
            ...
            edit_task_checklist_zip_url = $task_checklist
                .closest(".task_checklists_wrapper")
        ...
        $.ajax({
            ...
            success: function (data) {
                $task_checklist =
                    is_zipped === 1 ? $task_checklist.addClass("zipped") : $task_checklist.removeClass("zipped");
            },
        });
    });
    ...
    $body.on("keyup", ".new_task_checklist_item_input", function (event) {
        ...
        if (event.key === "Enter" && $this.val() !== "") $this.parent().next().find("a").trigger("click");
    });
    ...
    $body.on("click", ".save_new_task_checklist_item", function (event) {
        let $this = $(this),
            ...
            $task_checklist_item_input = $this
                .closest(".new_task_checklist_item")
                .find(".new_task_checklist_item_input"),
    ...
    $body.on("stop.uk.sortable", ".task_checklist_items", function (item) {
        let $this = $(item.detail[1]),
            ...
            edit_task_checklist_item_order_url = $(".task_checklists_wrapper").data(
                "edit_task_checklist_item_order_url",
            ),
...
var recalibrate_task_checklist_progress = function ($checklist) {
    ...
    if (checklist_items_total === 0 || checklist_items_done_total === 0) checklist_item_width_percent = "0%";
